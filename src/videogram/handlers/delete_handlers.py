...
async def delete_get_id(update, context):
    ...
    if result:
        ...
        if str(update.effective_user.id) != context.user_data['video'].user_id and \
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text=_("delete_no_permissions"))
            return ConversationHandler.END
...
async def on_chosen_delete_option(update, context):
    ...
    if update.callback_query.data == 'yes':
        try:
            ...
            orm.session.commit()
            await update.callback_query.edit_message_caption(_("delete_ok"))
        except Exception as e:
            print(e)
            await update.callback_query.edit_message_caption(_("error"))

    elif update.callback_query.data == 'no':
        await update.callback_query.edit_message_caption(_("cancel_command"))
    else:
        await update.callback_query.edit_message_caption(_("unknown_command"))
